...
# Search zones for temperature mapping
cpu_thermals = [
...
class CPU:
	...
	def get_temperatures(self):
		...
		if not temps:
			...
			if temps:
				max_val = max(val[0] for val in temps.values() if val[0] is not None)
				temps["Tctl"] = [max_val, None]
	...
	def get_cpu_info():
		...
		if cpu_info:
			...
			# Try getting the SoC name using Android properties or sysfs
			if not cpu_model:
				soc_model = ""
				for prop in ["ro.soc.model", "ro.board.platform", "ro.hardware.chipname", "ro.hardware"]:
					try:
						val = subprocess.check_output(["getprop", prop], stderr=subprocess.DEVNULL).decode().strip()
						if val:
							soc_model = val
							break
					except:
				...
				if not soc_model:
					for path_soc in ["/sys/devices/soc0/soc_id", "/sys/devices/soc0/machine", "/sys/devices/soc0/family"]:
						if exists(path_soc):
							val = get(path_soc)
							if val:
								soc_model = val
				...
				processor_count = sum(1 for line in cpu_info.splitlines() if line.strip().startswith("processor"))
				if soc_model:
					soc_name = soc_model.upper() if len(soc_model) < 15 else soc_model
					cpu_model = f"{soc_name} ({processor_count} cores)"
				elif processor_count > 0:
					cpu_model = f"Android CPU ({processor_count} cores)"
			else:
				processor_count = sum(1 for line in cpu_info.splitlines() if line.strip().startswith("processor"))

			# On Android/ARM, cpu cores matches physical cores (no SMT)
			if cores == 1 and processor_count > 0:
...
def get_android_temps():
	...
	for zone in zones:
		...
		if not any(keyword in zone_type_lower for keyword in ["cpu", "tsens", "big", "little", "g3d", "soc"]):
			continue
